...
class Task(Base):
    ...
    created_by: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    assignee_id: Mapped[UUID | None] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
    ...
    assignee_workload: Mapped[float | None] = mapped_column(Float, nullable=True)  # 0.0–1.0 capacity lúc assign
    ...
    deadline_buffer_hrs: Mapped[float | None] = mapped_column(Float, nullable=True)  # Giờ còn lại khi assign
    first_status_change_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)

    # ─── NLP Output (Phase 3 sẽ điền) ───────────────────────────────────────
    tags: Mapped[list[str] | None] = mapped_column(ARRAY(String), nullable=True)  # ["Bug", "Frontend"]
